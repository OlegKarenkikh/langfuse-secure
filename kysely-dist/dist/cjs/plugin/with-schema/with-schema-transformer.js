...
const using_node_js_1 = require("../../operation-node/using-node.js");
...
class WithSchemaTransformer extends operation_node_transformer_js_1.OperationNodeTransformer {
    ...
    #collectSchemableIdsFromTableExpr(node, schemableIds) {
        ...
        if (alias_node_js_1.AliasNode.is(node) && table_node_js_1.TableNode.is(node.node)) {
            return this.#collectSchemableId(node.node.table, schemableIds);
        }
        if (list_node_js_1.ListNode.is(node)) {
            for (const table of node.items) {
                this.#collectSchemableIdsFromTableExpr(table, schemableIds);
            }
            return;
        }
        if (using_node_js_1.UsingNode.is(node)) {
