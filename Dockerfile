...
# Устанавливаем pnpm + скачиваем патч-версии + ресинхронизируем через rsync — ВСЕ в одном RUN-слое.
# Trivy не увидит старый tar, т.к. патч происходит в том же слое, что и npm install -g pnpm.
RUN set -e; \
